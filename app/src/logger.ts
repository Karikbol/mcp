...
export const logger = pino({
  ...
  serializers: {
    req: (req: { headers?: Record<string, string>; body?: unknown }) => {
      ...
      if (headers?.["x-telegram-init-data"]) headers["x-telegram-init-data"] = "***MASKED***";
      if (headers?.["X-Telegram-Init-Data"]) headers["X-Telegram-Init-Data"] = "***MASKED***";
      const body = req?.body as Record<string, unknown> | undefined;
      if (body?.initData) body.initData = "***MASKED***";
      return {
        method: (req as Record<string, unknown>).method,
        url: (req as Record<string, unknown>).url,
        headers,
        body: req?.body ? maskObject((req.body as Record<string, unknown>) ?? {}) : undefined,
      };
    },
  },
});
